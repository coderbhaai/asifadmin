<div class="container.admin mb-10">
    <div class="flex grid grid-cols-12 gap-4 gap-x-3">
        ...
        <div class="col-span-12 md:col-span-10 px-3 md:pt-10">
            <h1 class="text-center text-3xl font-bold py-2 mb-6">Admin Meta </h1>
            <div class="md:flex justify-between mb-2 md:mb-0">
                <button wire:click="openModal()" class="w-full md:w-auto bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded my-3">Add Meta</button>
                <div class="row">
            ...
            <table class="min-w-full table-auto mb-5">
                ...
                <tr class="bg-primary">
                    ...
                    <th class="py-2 relative hover:cursor-pointer" wire:click="sortBy('url')"><span class="text-white">URL</span> @include('partials.sorticon', ['field'=>'url']) </th>
                    <th class="py-2 relative hover:cursor-pointer" wire:click="sortBy('title')"><span class="text-white">Title</span> @include('partials.sorticon', ['field'=>'title'])</th>
                    <th class="py-2 relative hover:cursor-pointer" wire:click="sortBy('description')"><span class="text-white">Description</span> @include('partials.sorticon', ['field'=>'description'])</th>
                ...
                <tbody class="">
                        @foreach($data as $i)
                            <tr class="">
                                ...
                                <td class="px-1 py-1">{{$i->url}}</td>
                                <td class="{{ strlen( $i->title ) < 30 || strlen( $i->title ) > 60 ? 'bg-action text-white ' : 'px-1 py-1' }}">{{$i->title}} - {{ strlen( $i->title ) }}</td>
                                <td class="{{ strlen( $i->description ) < 130 || strlen( $i->description ) > 160 ? 'bg-action text-white ' : 'px-1 py-1' }}">{{$i->description}} - {{ strlen( $i->description ) }}</td>
