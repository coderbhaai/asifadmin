<div class="container.admin mb-10">
    <div class="flex grid grid-cols-12 gap-4 gap-x-3">
        ...
        <div class="col-span-12 md:col-span-10 px-3 md:pt-10">
            <h1 class="text-center text-3xl font-bold py-2 mb-6">Admin Comments </h1>
            ...
            <table class="min-w-full table-auto mb-5">
                ...
                <tr class="bg-primary">
                    ...
                    <th class="py-2 relative hover:cursor-pointer" wire:click="sortBy('url')"><span class="text-white">Blog</span> @include('partials.sorticon', ['field'=>'url']) </th>
                    <th class="py-2 relative hover:cursor-pointer" wire:click="sortBy('title')"><span class="text-white">Comment</span> @include('partials.sorticon', ['field'=>'title'])</th>
                    <th class="py-2 relative hover:cursor-pointer" wire:click="sortBy('description')"><span class="text-white">Status</span> @include('partials.sorticon', ['field'=>'description'])</th>
                ...
                <tbody class="">
                    @foreach($data as $i)
                        <tr class="">
                            ...
                            <td class="px-1 py-1"><a href="{{$i->url}}" target="_blank">{{$i->title}}</a></td>
                            <td class="px-1 py-1">{{$i->comment}}</td>
                            <td class="px-1 py-1">@if($i->status==0) Hide @else Show @endif </td>
                ...
                <div class="relative sm:w-3/4 mx-2 sm:mx-auto my-10 opacity-100">
                    ...
                    x-transition:enter="transition transform duration-300" x-transition:enter-start="scale-0" x-transition:enter-end="scale-100" x-transition:leave="transition transform duration-300" x-transition:leave-start="scale-100" x-transition:leave-end="scale-0">
                        <header class="flex items-center justify-between p-2">
                            <h2 class="font-semibold">Update Comment Here</h2>
                                ...
                                <div class="bg-white">
                                    <div class="grid grid-cols-12 gap-6">
                                        <div class="col-span-4">
                                            <label for="user" class="block text-sm font-medium text-gray-700">Name</label>
                                            <input type="text" wire:model="user" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Add Name">
                                        ...
                                        <div class="col-span-12">
                                            ...
                                            <textarea type="text" wire:model="comment" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Add Comment"></textarea>
