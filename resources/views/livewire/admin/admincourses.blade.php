<div class="container.admin">
    <div class="flex grid grid-cols-12 gap-4 gap-x-3">
        ...
        <div class="col-span-12 md:col-span-10 px-3 md:pt-10">
            <h1 class="text-center text-3xl font-bold py-2 mb-6">Admin Courses</h1>
            <div class="md:flex justify-between mb-2 md:mb-0">
                <a href="{{ route('addcourse') }}" class="block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded my-3">Add Course</a>
                <div class="row">
            ...
            <table class="min-w-full table-auto mb-5">
                ...
                <tr class="bg-primary">
                    ...
                    <th class="py-2 relative hover:cursor-pointer" wire:click="sortBy('name')"><span class="text-white">Course</span> @include('partials.sorticon', ['field'=>'name'])</th>
                    <th class="py-2 relative"><span class="text-white">Image</span></th>
                    <th class="py-2 relative"><span class="text-white">Price</span></th>
                ...
                <tbody class="">
                    @foreach($data as $i)
                        <tr class="">
                            ...
                            <td class="px-1 py-1"><a href="/course/{{$i->url}}" target="_blank">{{$i->name}}</a></td>
                            <td class="px-1 py-1"><img src="/storage/course/{{$i->image}}" class="w-32"/></td>
                            <td class="px-1 py-1">{{$i->price}} | {{$i->sale}}</td>
                            <td class="px-1 py-1">@if($i->status===1) Active @else Not Active @endif </td>
                            <td class="px-1 py-1">{{date('d-m-Y', strtotime($i->updated_at))}}</td>
                            <td class="px-1 py-1">
                                <a class="bg-indigo-500 text-white px-4 py-1 border rounded-md hover:bg-white hover:border-indigo-500 hover:text-black" href="/admin/updatecourse/{{$i->id}}">Edit</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
